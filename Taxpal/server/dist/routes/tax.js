...
const calculateTax = (income, country) => {
...
router.post('/calculate', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        let taxEstimate = await TaxEstimate_1.default.findOne({
            user_id: req.user._id,
        ...
        if (taxEstimate) {
            ...
            // Create new estimate
            taxEstimate = new TaxEstimate_1.default({
                user_id: req.user._id,
                quarter: currentQuarter,
                year: currentYear,
...
router.get('/calendar', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const calendar = await Promise.all(quarters.map(async (quarter) => {
            const dueDate = getQuarterDueDate(currentYear, quarter);
            const estimate = await TaxEstimate_1.default.findOne({
                user_id: req.user._id,
                quarter,
                year: currentYear
            });
...
function getQuarterDueDate(year, quarter) {
    const dueDateMap = {
